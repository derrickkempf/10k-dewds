...
// Lazy-instantiate so missing env var at build/collect time doesn't blow up.
let _resend = null
function getResend() {
...
const FROM = process.env.FROM_EMAIL || 'DEWDS <onboarding@resend.dev>'

export async function sendVerificationEmail(email, code) {
  const { error } = await getResend().emails.send({
    from: FROM,
    to: email,
    subject: `${code} — your DEWDS verification code`,
    html: `
      <div style="font-family:-apple-system,BlinkMacSystemFont,sans-serif;max-width:400px;margin:0 auto;padding:40px 0">
        <p style="font-size:14px;color:#999;margin:0 0 24px">DEWDS</p>
